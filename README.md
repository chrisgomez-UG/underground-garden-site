...
- **Forms (Web3Forms)** — `WEB3FORMS_ACCESS_KEY` in `assets/js/main.js` is a real key, tested end-to-end with a real submission. All 6 forms (bookings, partnerships, general/press, 3 signup forms) post to Web3Forms, which emails `chris.gomez@ugevents.com` (the Web3Forms account owner; the site's displayed contact `info@ugevents.com` CCs there). Each form sends a different `data-form-subject` so incoming emails are easy to tell apart. If the key ever needs rotating, it's the one constant near the top of the form-handling block in `main.js` — was `"PASTE_YOUR_ACCESS_KEY_HERE"` as a placeholder, now a real UUID.
  ...
  - **Form note placement:** the three inline signup forms (Home/Subtropics/Merch) keep their `[data-form-note]` status line *outside* the `<form>` tag; `main.js` looks inside the form first, then in the form's parent. (Before 2026-07-14 it only looked inside the form — those three forms showed **no** success/failure message at all, a real shipped bug.) The note also carries `role="status"` so screen readers announce the result, and failures get a `.form-note--error` rust-colored treatment instead of the default green success box.
- **SMS signup (not email)** — the 3 "Get On The List" forms (Home, Subtropics presale, Merch) collect a **phone number** (`type="tel"`, with SMS consent microcopy), not an email address — changed 2026-07-03 to match how DEF (defineeverythingfuture.com) runs their list, since text has better open rates than email for time-sensitive drops. These still submit through Web3Forms same as every other form (into `chris.gomez@ugevents.com` + the submissions Sheet, see below) — **there is no automated texting integration, and none is planned.** Chris uses [ExpertTexting](https://www.experttexting.com/) for actually sending SMS blasts and manually uploads numbers there himself from the collected submissions. Don't build or suggest an ExpertTexting API integration unless he asks — this is intentionally a manual step on his end.
...
- `assets/img/brand/og-image.jpg` (1200x630) is the social-share preview image — a branded card generated from `wordmark.png` on the hero gradient colors. Swap it for a real event photo card if you want something less generic later.
...
- Every slot is the same uniform size — `<div class="media-card media-card--flyer tilt-a|b reveal"><img src="assets/img/events/FILE.jpg" alt="..." loading="lazy" width="W" height="H" /></div>` — alternate `tilt-a`/`tilt-b` for the subtle stagger look. **(Changed 2026-07-10, Chris's call:** this grid used to feature every 3rd flyer at 2x size via `media-card--featured` + a `.media-card-pair` wrapper for its neighbors — removed for a simpler, uniform grid. If you see either of those classes referenced anywhere, that pattern is gone; don't reintroduce it without Chris asking.)
...
- Added because the site collects real PII (name, email, phone number) across 6 forms for email/SMS marketing — SMS platforms and ad/analytics platforms generally expect a linked privacy policy, and it's a baseline credibility/trust signal regardless.
...
- `netlify.toml` also carries (added 2026-07-14, live only on Netlify — the
  ...
  - **Security headers** on every path (X-Frame-Options, nosniff,
    Referrer-Policy, Permissions-Policy, HSTS). No Content-Security-Policy
    yet — the pages use inline scripts/styles, so a meaningful CSP needs
    `unsafe-inline` allowances thought through first; flagged as a future
    hardening step, don't add one casually.
